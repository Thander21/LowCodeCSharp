...
using LowCodeCSharp.Analyzer;
using LowCodeCSharp.Diagram;
using LowCodeCSharp.Visualizer;
...
namespace LowCodeCSharp
{
    public class LowCodeCSharp
    {
        static void Main(string[] args)
        {
            ...
            DiagramGenerator diagramGenerator = new DiagramGenerator(flowNodes, flowConnections);
            diagramGenerator.GenerateDiagram();

            // Inicia a visualização do diagrama (opcional)
            VisualizerScreen visualizer = new VisualizerScreen();
            visualizer.ShowDiagram(flowNodes, flowConnections);
        }

        // Método para gerar as conexões de fluxo
        static List<FlowConnection> GenerateFlowConnections(List<FlowNode> flowNodes)
        {
